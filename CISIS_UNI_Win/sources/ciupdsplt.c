...
#define MAXNWORDS 4000
            ...
            if (*p == '=') {
                ...
                if (strncmp(p,"words",5) == 0) {
                  int i;
#if UTF8
                  for (i=0; i<sizeof(tabsplit); i++)
                  {
                  	char	texto[2];
                  	sprintf(texto,"%c",i);
					tabsplit[i] = 1 - ( alphabetic_utf8(texto) ? 1 : 0 );
                  }
#else
...
#if UTF8
	                  	char	texto[2];
    	              	sprintf(texto,"%c",*fldq);
						uppercase_utf8(texto);
						uc = texto[0];
#else
                      ...
                      if (addp) {
                        ...
                        if (addlen) {
                          if (trigrams) { /* trigrams */
                            ...
                            /* upper case */
#if UTF8
							up = uppercase_utf8_length(isisuctab_utf8,addp,addlen);
#else
