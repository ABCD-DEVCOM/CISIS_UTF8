...
>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> */
   ...
   findsize = strlen(key);
    ...
    /* 2 */
#if UTF8
	for ( p = comp, pos = 0L; *p; q = skipchar_utf8(p,1), pos += (q - p), p = q )
	{
		/* 3 */
		if ( *p != *key ) continue;
		...
		if ( pos )
		{
			previous_p = previous_char_utf8(pos,p);
			if ( previous_p )
				if ( alphabetic_utf8(previous_p) ) continue;      	
		}
		if ( !truncate )
			if ( *(p+findsize) )
				if ( alphabetic_utf8(p + findsize) ) continue;
