...
#if UTF8
STRUCT_ISISUCTAB_UTF8 * easy_uctab_utf8 = 0;	// utf-8 cisis upper case table
STRUCT_ISISACTAB_UTF8 * easy_actab_utf8 = 0;	// utf-8 cisis alphabetic table

/* =============================================================== get_line */
char *	get_line				// get next line from a string buffer
(
	char *	p,					// pointer for each byte
	char 	line[],				// target line
	int		line_size			// maximum available bytes in the target line
)
{
	char *	pEnd;				// pointer to the line end
	int		n;					// line length
	
	// find the end of line
	pEnd = strchr(p,'\n');
	
	// assign the line length
	if ( !pEnd ) n = strlen(p);
	else n = pEnd - p;
	if ( n > line_size ) n = line_size;
	
	// copy line
	strncpy(line,p,n);
	line[n] = 0;

	// find the begin of the next line
	for ( p = pEnd; p; p++ )
	{
		if  ( !*p ) break;
		if  ( strchr("\n\r",*p) == 0 ) break;
...
{
	STRUCT_ISISUCTAB_UTF8 *	uctab_utf8;		// new upper case table area
    int						loop;			// line count
    char					line[BUFSIZ];	// line buffer
    unsigned char *			next_line;		// pointer to the next line
    unsigned char *			p;				// pointer to the line buffer characters
	int						num;			// decimal number
    int 					i;				// number buffer count
    int						col;			// upper case table column
    int						line_qtt;		// quantity of lines

	// count number of lines
	for ( line_qtt = 0, next_line = area; next_line; )
	{
		if ( !*next_line ) break;									// quit on end of buffer							
		next_line = get_line(next_line,line,sizeof(line) - 1);		// get next line
		if ( line[0] == '#' ) continue;								// ignore comments
		if ( strchr(line,'=') == 0 ) continue;						// ignore line without assignment
		line_qtt++;													// count line
	}
	line_qtt++;														// consider last null column

	// allocate tabel area
	uctab_utf8 = (STRUCT_ISISUCTAB_UTF8 *)ALLOC(sizeof(STRUCT_ISISUCTAB_UTF8) * line_qtt);
    if ( !uctab_utf8 ) return efc_error(err,ECI_ERROR_UCTAB,"loaductb_utf8/alloc");
    memset(uctab_utf8,0,sizeof(STRUCT_ISISUCTAB_UTF8) * line_qtt);	// reset all table
	
	// all lines
    for ( loop=0, next_line = area; next_line; )
	{
		if ( !*next_line ) break;									// check end of buffer
		next_line = get_line(next_line,line,sizeof(line) - 1);		// get next line
		if ( line[0] == '#' ) continue;								// ignore comments
		if ( loop >= line_qtt ) return efc_error(err,ECI_ERROR_UCTAB,"loaductb_utf8/max lines");	// just in case
		...
		{
			// get decimal number
			p = get_decimal_number(p,&num);
			
			// validation
			if ( col > 3 ) return efc_error(err,ECI_ERROR_UCTAB,"loaductb_utf8/max 4 columns");
			
			// store decimal number that represents the character
			uctab_utf8[loop].lower[col] = num;
		...
		if ( uctab_utf8[loop].lower[0] == 0 ) continue;
		
		// check if the table is ordered
		if ( loop > 0 )
		{
			if ( strcmp(uctab_utf8[loop].lower,uctab_utf8[loop - 1].lower) < 0 )
				return efc_error(err,ECI_ERROR_UCTAB,"loaductb_utf8/it is not in ascending order");
		}
		...
		do
		{
			...
			// store decimal number that represents the character
			uctab_utf8[loop].upper[col] = num;
			
			// validation
			if ( uctab_utf8[loop].upper[col] > 0 && uctab_utf8[loop].lower[col] == 0 ) 
				return efc_error(err,ECI_ERROR_UCTAB,"loaductb_utf8/right columns exceeds left columns");
	...
	// assign new allocated table
	if ( easy_uctab_utf8 ) FREE(easy_uctab_utf8);
	easy_uctab_utf8 = uctab_utf8;
	isisuctab_utf8 = easy_uctab_utf8;
...
{
	STRUCT_ISISACTAB_UTF8 * actab_utf8;	// new alphavetic table area
    int						loop;			// line count
    char					line[BUFSIZ];	// line buffer
    unsigned char *			next_line;		// pointer to the next line
    unsigned char *			p;				// pointer to the line buffer characters
    ...
    if ( !actab_utf8 ) return efc_error(err,ECI_ERROR_UCTAB,"loadactb_utf8/alloc");
    ...
    for ( loop=0, next_line = area; next_line; )
    {
		...
		next_line = get_line(next_line,line,sizeof(line) - 1);		// get next line
		if ( line[0] == '#' ) continue;								// ignore comments
		if ( loop >= line_qtt ) return efc_error(err,ECI_ERROR_UCTAB,"loadactb_utf8/max lines");	// just in case
		...
		do
		{
			...
			// validation
			if ( col > 3 ) return efc_error(err,ECI_ERROR_UCTAB,"loadactb_utf8/max 4 columns");
			
			// store decimal number that represents the character
			actab_utf8[loop].code[col] = num;
		...
		// check if it is a valid number
		if ( actab_utf8[loop].code[0] == 0 ) continue;
		
		// check if the table is ordered
		if ( loop > 0 )
		{
			if ( strcmp(actab_utf8[loop].code,actab_utf8[loop - 1].code) < 0 )
				return efc_error(err,ECI_ERROR_UCTAB,"loadactb_utf8/it is not in ascending order");
		}
	...
	// assign new allocated table
	if ( easy_actab_utf8 ) FREE(easy_actab_utf8);
	easy_actab_utf8 = actab_utf8;
	isisactab_utf8 = easy_actab_utf8;
