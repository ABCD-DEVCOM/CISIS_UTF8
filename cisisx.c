...
#define CIAOTWCT 1
    ...
    if (!isiswctot) {
#if UTF8
	isiswctot=1;	// isiswctot different from zero means dbxinit was done
#else
...
#define GETACUCTAB 1
    ...
    if (dbxcipok) {
#if UTF8
       	isisuctab_utf8 = loaductb_utf8(NULL,p);
    ...
    if (dbxcipok) {
#if UTF8
	       	isisactab_utf8 = loadactb_utf8(NULL,p);
    ...
    *areap='\0';
	...
	for (--asize, p=(unsigned char *)areap; ; )
	    if (asize > 0) {
			...
			{
				int iHeader;
				int	lenHeader = strlen(file_header_utf8);
				
				for ( iHeader = 0; iHeader < lenHeader && rc == file_header_utf8[iHeader]; iHeader++ )
				{
					rc = fgetc(fp);
				}
	   			if ( rc == EOF) break;
...
#endif /*CICPP*/
{
    ...
    *nstws=0;
	...
	if ( header )
	{
		unsigned char * pLineBegin;

		pLineBegin = skip_header_file_utf8(line);
		strcpy(line,pLineBegin);
		header = 0;	
	}
#endif // UTF8
	len=strlen(line);
